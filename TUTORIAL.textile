...
<% @user.subdomains.each do |subdomain| %>
  <tr>
    ...
    <td><%= link_to 'Edit', edit_subdomain_path(subdomain) %></td>
    <td><%= link_to 'Destroy', subdomain, :confirm => 'Are you sure?', :method => :delete %></td>
    <td><%= link_to "Visit #{site_url(:subdomain => subdomain.name)}", site_url(:subdomain => subdomain.name) %></td>
  </tr>
<% end %>
</table>
<br />
...
*Note:* Using @:domain => :all@ gives us the error "ActionController::InvalidAuthenticityToken in Devise/sessionsController#create" when we attempt to log in. 
