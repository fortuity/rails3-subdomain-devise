...
h4. Issues With Cookies in the Rails 3 Release Candidate

The Rails 3 release candidate does not behave as expected when @:domain => :all@ is set in *config/initializers/sesion_store.rb*.

Using @:domain => :all@ gives us the error "ActionController::InvalidAuthenticityToken in Devise/sessionsController#create" when we visit "http://lvh.me:3000/":http://lvh.me:3000/ and attempt to log in. This is not an issue with Devise. Creating and submitting any form generates the "ActionController::InvalidAuthenticityToken" error.

*Note:* Using @:domain => '.lvh.me'@ doesn't work. It won't allow us to maintain sessions across subdomains. Looks like "an open bug":https://rails.lighthouseapp.com/projects/8994/tickets/5147-the-all-domain-option-for-the-cookie-session-store-doesnt-allow-non-standard-tlds-like-local-or-couk in the Rails 3 release candidate.
